  ...
  return (
    <section className="panel">
      <div className="section-head">
        ...
        <Link className="text-link" href="/students?risk=warning">Tüm öğrencileri aç</Link>
      </div>
      <table className="data-table">
        <thead>
          <tr>
        ...
        <tbody>
          {students.map((student) => (
            <tr key={student.id}>
              ...
              <td><Link className="text-link" href={`/students/${student.id}`}>Detayı aç</Link></td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <tbody>
          {students.map((student) => (
            <tr key={student.id}>
              <td>
                <div className="table-primary">
                  <Link className="text-link strong-link" href={`/students/${student.id}`}>{student.name}</Link>
                ...
                <div key={assignment.id} className="assignment-card">
                  ...
                  <div className="assignment-meta">
                    ...
                    <div style={{ display: "flex", gap: 12 }}>
                      <Link className="text-link" href={`/assignments/${assignment.id}`}>
                        Detayı aç
                      </Link>
                      <Link className="text-link" href={`/students?classId=${assignment.classId}&assignmentId=${assignment.id}`}>
                        Tamamlamayanlar
                      </Link>
                    </div>
                  </div>
        ...
        <tbody>
          {classes.map((item) => (
            <tr key={item.id}>
              ...
              <td>{formatInteger(item.activeStudents)}/{formatInteger(item.studentCount)}</td>
              <td>{formatPercent(item.report.completionRate)}</td>
              <td>{formatInteger(item.atRiskStudents)} öğrenci</td>
              <td><Link className="text-link" href={`/classes/${item.id}`}>Raporu aç</Link></td>
            </tr>
          ))}
        </tbody>
      </table>
